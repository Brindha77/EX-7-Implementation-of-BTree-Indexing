...
<div align="justify">
Step 1: Starting from the root node, compare k with the first key of the node.<br>
  
Step 2: If k = the first key of the node, return the node and the index.<br>

Step 3: If k.leaf = true, return NULL (i.e. not found).<br>
If k < the first key of the root node, search the left child of this key recursively.<br>

Step 4: If there is more than one key in the current node and k > the first key, compare k with the next key in the node.
